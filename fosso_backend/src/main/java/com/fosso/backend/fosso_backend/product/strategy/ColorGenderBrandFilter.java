...
public class ColorGenderBrandFilter implements ProductFilterStrategy {
    ...
    public Page<Product> filter(ProductFilterCriteria criteria, Pageable pageable) {
        return productRepository.findByColorAndGenderAndBrandIdForUser(
                criteria.getColor(), criteria.getGender(), criteria.getBrandId(), pageable);
    }
}