...
public class GenderCategoryPriceRangeFilter implements ProductFilterStrategy {
    ...
    public Page<Product> filter(ProductFilterCriteria criteria, Pageable pageable) {
        return productRepository.findByGenderCategoryAndPriceRangeForUser(
                criteria.getGender(), criteria.getCategoryIds(), criteria.getMinPrice(), criteria.getMaxPrice(), pageable);
    }
}