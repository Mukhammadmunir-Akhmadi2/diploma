...
public class ColorGenderCategoryBrandFilter implements ProductFilterStrategy {
    ...
    public Page<Product> filter(ProductFilterCriteria criteria, Pageable pageable) {
        return productRepository.findByColorGenderCategoryBrandForUser(
                criteria.getColor(), criteria.getGender(), criteria.getCategoryIds(), criteria.getBrandId(), pageable);
    }
}