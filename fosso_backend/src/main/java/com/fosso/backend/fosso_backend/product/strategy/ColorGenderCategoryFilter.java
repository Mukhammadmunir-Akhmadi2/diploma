...
public class ColorGenderCategoryFilter implements ProductFilterStrategy {
    ...
    public Page<Product> filter(ProductFilterCriteria criteria, Pageable pageable) {
        return productRepository.findByColorAndGenderAndCategoryIdForUser(
                criteria.getColor(), criteria.getGender(), criteria.getCategoryIds(), pageable);
    }
}