...
public class KeywordGenderFilter implements ProductFilterStrategy {
    ...
    public Page<Product> filter(ProductFilterCriteria criteria, Pageable pageable) {
        return productRepository.findByKeywordAndGenderForUser(criteria.getKeyword(), criteria.getGender(), pageable);
    }
}