...
public class ColorPriceRangeFilter implements ProductFilterStrategy {
    ...
    public Page<Product> filter(ProductFilterCriteria criteria, Pageable pageable) {
        return productRepository.findByColorAndPriceRangeForUser(
                criteria.getColor(), criteria.getMinPrice(), criteria.getMaxPrice(), pageable);
    }
}